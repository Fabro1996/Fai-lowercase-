...
HOW TO MATCH:
1. Look at the balloon x/y coordinate on the drawing image
2. Find the dimension callout, leader line, or annotation at or near that location
3. Match it to the characteristic whose description AND nominal value best match what you see at that spot
4. Use the matched characteristic's description, nominal, unit, and tolerance exactly as given — do not paraphrase or rename
5. Each characteristic can only be matched to ONE balloon — do not assign the same characteristic to two balloons
6. If a balloon is placed next to "6XØ12.7", match it to the hole diameter characteristic, not a nearby different dimension

MATCHING PRIORITY (in order):
1. matchingText match: if the balloon is next to text that exactly matches a characteristic's matchingText (e.g. "6XØ12.7", "4XR6.35", "73.03"), that is the correct match
2. Nominal + type match: if a value is visible at the location, find the characteristic with that nominal value and the matching dimension type
3. Spatial: among remaining unmatched characteristics, pick the one whose callout is geometrically closest to the balloon position
