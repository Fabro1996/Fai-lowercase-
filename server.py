        ...
        raw = call_claude([
            {
                ...
                "content": [
                    ...
                    {
                        "type": "text",
                        "text": f"""You are a metrologist reading an engineering drawing to identify dimensions at specific locations.

The image coordinate system: x=0% is the left edge, x=100% is the right edge, y=0% is the top edge, y=100% is the bottom edge.

General tolerances from title block:
{json.dumps(tolerances)}

For each balloon position below, look at that location on the drawing and identify:
1. What dimension callout or annotation is nearest to that point
2. The exact numeric value shown
3. The tolerance (from the dimension itself, or from the title block general tolerance table if not specified)
4. A precise metrology description using proper GD&T and inspection language

Balloon positions:
{pos_list}

DESCRIPTION RULES — use proper metrology language:
- "Overall length", "Overall width", "Overall height"
- "Sheet thickness", "Wall thickness", "Plate thickness"
- "Hole diameter Nx (ØX.XX)" where N is quantity
- "Hole center to cut edge — [side]"
...
- "Flange height", "Flange width"
- "Chamfer X x X°"
- "True position Ø X.XX |datum refs|"
- "Flatness X.XX — [surface]"
...
TOLERANCE RULES:
- Use the specific tolerance shown on that dimension if visible
- If no specific tolerance shown, apply the correct tier from the general tolerance table
- Split ±X.XX into tolPlus="+X.XX" and tolMinus="-X.XX"
- GD&T geometric tolerances: tolPlus="X.XX", tolMinus="0.00"
- Surface finish / material: tolPlus="", tolMinus=""
- If you cannot confidently identify a dimension at a position, use desc="Unidentified — verify manually", nominal="", tolPlus="", tolMinus=""

NOTES — add inspection method where relevant:
- Caliper, micrometer, CMM, optical comparator, height gauge, Ra profilometer, visual, cert required
...
Every balloon must have exactly one row. Total rows: {len(positions)}"""
                    },
                ],
            }
        ], max_tokens=4000)
