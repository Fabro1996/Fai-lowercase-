...
def identify_balloons():
    ...
    try:
        ...
        # Characteristics extracted during /api/analyze — used for matching
        raw_chars = request.form.get("characteristics")
        characteristics = json.loads(raw_chars) if raw_chars else []

        # Build position + characteristics list for the prompt
        pos_list = "\n".join(
        ...
        char_list = "\n".join(
            f"  ID {c['id']}: {c['description']} | nominal={c['nominal']} {c.get('unit','')} | tol={c.get('tolerance','')} | matchingText={c.get('matchingText','')}"
        ...
        raw = call_claude([
            {
                ...
                "content": [
                    ...
                    {
                        "type": "text",
                        "text": f"""You are a senior metrologist matching balloon positions on an engineering drawing to their corresponding dimensions.

COORDINATE SYSTEM: x=0% left edge, x=100% right edge, y=0% top edge, y=100% bottom edge.

═══ ALREADY-EXTRACTED CHARACTERISTICS ═══
The following characteristics were already extracted from this drawing by reading the text:
{char_list}

═══ BALLOON POSITIONS TO MATCH ═══
{pos_list}

═══ DRAWING LAYOUT ═══
This drawing is a standard sheet metal multi-view drawing. Use these approximate regions to guide matching:

TOP VIEW (plan view):     x=15–55%, y=8–35%   — shows overall plate outline, overall length/width
FRONT VIEW:               x=15–48%, y=35–65%  — shows holes, hole spacing, edge distances, hole diameters, corner radii
SIDE/SECTION VIEW:        x=48–72%, y=35–65%  — shows the bent profile: flange height, slot width, slot height, slot corner radii, sheet thickness, and the "95.25" dimension appears here as the overall width in this view
ISOMETRIC VIEW:           x=58–92%, y=8–40%   — decorative 3D view, ignore for matching
TITLE BLOCK:              x=55–92%, y=65–92%  — ignore for matching
TECHNICAL NOTES:          x=8–48%,  y=68–88%  — ignore for matching

When a balloon is in the SIDE/SECTION VIEW region (x=48–72%, y=35–65%), only match it to characteristics that would appear in that view: slot dimensions, flange/leg heights, sheet thickness, bend radii, and the overall width shown in that view.

When a balloon is in the FRONT VIEW region (x=15–48%, y=35–65%), only match it to characteristics visible there: hole diameter, corner radius, hole center spacing, hole center to edge.

═══ YOUR JOB ═══
For each balloon position, look at that location on the drawing and determine which characteristic from the list above is closest to or at that position.

HOW TO MATCH:
1. Process each balloon IN THE EXACT ORDER they are listed above — balloon #1 first, balloon #2 second, etc.
2. For each balloon, determine which view region it is in using the layout above
3. Narrow the candidate characteristics to those that belong in that view
4. Find the dimension callout, leader line, or annotation at or near that location
5. Match it to the characteristic whose description AND nominal value best match what you see at that spot
6. Use the matched characteristic's nominal and tolerance exactly as given — do not alter values
7. Each characteristic can only be matched to ONE balloon — do not assign the same characteristic to two balloons

CRITICAL — RESPONSE ORDER:
Your rows array MUST be in the same order as the balloon positions list.
Row 1 = Balloon #1, Row 2 = Balloon #2, Row 3 = Balloon #3, etc.
Do NOT reorder rows based on spatial logic or characteristic ID order.
The n field in each row must match the balloon id exactly.

MATCHING PRIORITY (in order):
1. matchingText match: if the balloon is next to text that exactly matches a characteristic's matchingText (e.g. "6XØ12.7", "4XR6.35", "73.03", "2XR1.5"), that is the correct match
2. Nominal + view match: find the characteristic with that nominal value that belongs in the same view region as the balloon
3. Spatial: among remaining unmatched characteristics in the same view region, pick the one geometrically closest to the balloon
4. Never reuse a characteristic already matched to another balloon

TOLERANCE FORMAT:
- "±0.20" → tolPlus="+0.20", tolMinus="-0.20"
- GD&T "0.25" → tolPlus="0.25", tolMinus="0.00"
- Material/finish → tolPlus="", tolMinus=""

DEVICE — measurement device or method used:
Caliper · Micrometer · CMM · Height gauge · Optical comparator · Pin gauge · Go/no-go gauge · Ra profilometer · Visual · Cert required
Put the device name in the "notes" field — keep it short (e.g. "Caliper", "CMM", "Micrometer")

Unmatched balloon: desc="Verify manually", nominal="", tolPlus="", tolMinus="", notes=""
...
{{
  "rows": [
    {{
      ...
      "tolPlus": "+0.20",
      "tolMinus": "-0.20",
      "notes": "Caliper"  // measurement device
    }}
  ],
  "certs": ["Material Certification — AL5052-H32"]
}}

desc field is optional — omit it or leave blank. The balloon number is the reference.
Total rows: {len(positions)}. One row per balloon. No characteristic used twice."""
                    },
                ],
            }
        ], max_tokens=4000)
